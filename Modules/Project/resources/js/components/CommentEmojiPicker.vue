...
// Rút gọn từ Modules/Social/resources/js/components/SocialEmojiPicker.vue:
// bỏ hẳn phần sticker (panel emoji/sticker, STICKER_PACKS, SocialAnimatedSticker)
// — Task/Project chỉ cần emoji thường.
...
<template>
  <Teleport to="body">
    <div ref="root" class="comment-emoji-picker" :style="popupStyle" role="dialog" aria-label="Chọn emoji">
      <div class="comment-emoji-picker__tabs">
        <div class="comment-emoji-picker__tab-list hide-scrollbar">
          <button
            v-for="group in EMOJI_GROUPS"
            :key="group.key"
            type="button"
            class="comment-emoji-picker__tab"
            :class="{ 'comment-emoji-picker__tab--active': activeGroup === group.key }"
        ...
        <button type="button" class="comment-emoji-picker__close" aria-label="Đóng bảng emoji" @click="emit('close')">
          <AppIcon name="close" :size="14" />
        </button>
      </div>

      <div class="comment-emoji-picker__grid hide-scrollbar">
        <button
          v-for="(emoji, index) in currentEmojis()"
          :key="index"
          type="button"
          class="comment-emoji-picker__cell"
...
.comment-emoji-picker__tab-list {
  display: flex;
  align-items: center;
  gap: var(--space-1);
  flex: 1;
...
.comment-emoji-picker__tab--active {
  background: var(--color-primary-surface);
}

.comment-emoji-picker__close {
  margin-left: auto;
  border: none;
  background: none;
...
.comment-emoji-picker__cell:hover {
