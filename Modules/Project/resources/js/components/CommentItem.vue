...
import { applyOptimistic, cloneReactions, useReactionAction } from '../lib/useCommentReactionAction.js';
import { sanitizeCommentHtml } from '../lib/sanitizeCommentHtml.js';
import { mentionUserIdFromEvent } from '../lib/commentMentionClick.js';
import CommentComposer from './CommentComposer.vue';
import CommentImageGrid from './CommentImageGrid.vue';
import CommentReactionBursts from './CommentReactionBursts.vue';
import CommentReactionList from './CommentReactionList.vue';
import CommentReactionPicker from './CommentReactionPicker.vue';

// Rút gọn từ Modules/Social/resources/js/components/SocialCommentItem.vue:
// bỏ hẳn hashtag (mentionClick vẫn giữ, hashtagClick bỏ), sticker directive
// (v-social-stickers), và permission moderate (canModerate) — chỉ tác giả
// mới xoá được bình luận của mình, theo yêu cầu đã chốt.
...
const props = defineProps({
  ...
  /** Chỉ true ở comment gốc khi cha (ProjectDiscussionTab) xác định user có quyền ghim — reply không bao giờ ghim được. */
  canPin: { type: Boolean, default: false },
});

const emit = defineEmits(['count-changed', 'deleted', 'replied', 'open-wall', 'pin-changed']);
...
const localIsPinned = ref(Boolean(props.comment.is_pinned));
...
const showPinAction = computed(() => props.depth === 0 && props.canPin);
...
/** Chỉ tác giả bình luận mới thấy được nút xoá — không có nhánh moderate. */
...
async function setReaction(type, event) {
  ...
  const gen = nextGen();
  try {
    const { data } = await window.axios.post(`/api/project/comments/${props.comment.id}/reactions`, { type });
...
async function togglePin() {
  ...
  pinning.value = true;
  const next = !localIsPinned.value;
  try {
    const { data } = next
      ? await window.axios.post(`/api/project/comments/${props.comment.id}/pin`)
      : await window.axios.delete(`/api/project/comments/${props.comment.id}/pin`);
    localIsPinned.value = Boolean(data.comment?.is_pinned);
    emit('pin-changed', data.comment);
  } catch (error) {
    showClientToast('error', error?.response?.data?.message ?? 'Không thể thực hiện thao tác ghim.');
  } finally {
    pinning.value = false;
  }
}
...
async function confirmRemove() {
  ...
  deleting.value = true;
  try {
    const { data } = await window.axios.delete(`/api/project/comments/${props.comment.id}`);
...
<template>
  <div class="comment-item" :class="{ 'comment-item--reply': depth > 0 }">
    <div class="comment">
      ...
      <div class="comment__body">
        <div class="comment__bubble" :class="{ 'comment__bubble--pinned': localIsPinned }">
          ...
          <div v-if="imageAttachments.length > 0" class="comment__images">
            <CommentImageGrid compact :images="imageAttachments" />
          </div>
        ...
        <div class="comment__meta-row">
          <CommentReactionPicker compact :my-reaction="localMyReaction" @pick="setReaction">
            <button
              type="button"
              class="comment__like"
              :class="[
          ...
          <button v-if="canNestReplies" type="button" class="comment__reply-toggle" @click="toggleReply">
            Trả lời
          </button>
      ...
      <div v-if="showPinAction || canDeleteComment(comment)" class="comment__actions">
        <button
          v-if="showPinAction"
          type="button"
          class="comment__action"
          :class="{ 'comment__action--on': localIsPinned }"
          :aria-label="localIsPinned ? 'Bỏ ghim bình luận' : 'Ghim bình luận'"
          :disabled="pinning"
          @click="togglePin"
        >
    ...
    <CommentReactionBursts :bursts="bursts" />
    ...
    <CommentReactionList
      ...
      :endpoint="`/api/project/comments/${comment.id}/reactions`"
