<script setup>
//
// Dialog SỬA mẫu đánh giá — component riêng biệt, KHÔNG dùng chung state
// với dialog Tạo mới (EvaluationTemplateCreateDialog.vue). Tách từ
...
async function submitForm() {
  ...
  try {
    const { data } = await window.axios.put(`/api/evaluation/templates/${form.id}`, payload);
    emit('updated', data.template);
    showClientToast('success', `Đã lưu mẫu đánh giá "${data.template.name}".`);
    emit('update:open', false);
  } catch (err) {
...
watch(
  ...
  (isOpen) => {
    ...
    nextTick(() => document.getElementById('evtpl-edit-name')?.focus());
  },
);
</script>
      ...
      >
        <div class="evtpl-dialog__panel evtpl-dialog__panel--fill" role="dialog" aria-modal="true" aria-labelledby="evtpl-edit-title">
          <div class="evtpl-dialog__head">
            ...
            <h2 id="evtpl-edit-title" class="evtpl-dialog__title">Sửa mẫu đánh giá</h2>
