...
const COLUMNS = [
    ...
    {
        key: "use_in_evaluation",
        label: "Dùng trong ĐGNL",
        defaultOn: true,
        align: "center",
    },
...
const COL_KEY = "va-eval-criteria-columns-v7";
...
const filtered = computed(() => {
    ...
    return allCriteria.value.filter((c) => {
        if (
            ...
            !(c.criterion_type?.name ?? "").toLowerCase().includes(q) &&
            !(c.criterion_type?.code ?? "").toLowerCase().includes(q)
        )
            return false;
        if (
            kindFilter.value &&
            String(c.criterion_type_id) !== String(kindFilter.value)
        )
            return false;
...
const pagerMeta = computed(() => {
    ...
    const visible = criteriaGroups.value.reduce(
        (n, g) =>
            n +
            (isTypeGroupCollapsed(typeGroupCollapsed, g.key)
                ? 0
                : g.criteria.length),
        0,
    );
...
const selectedDialogCriterion = computed(
    () =>
        allCriteria.value.find((item) => String(item.id) === String(form.id)) ??
        null,
);
...
function typeOrderIndex(typeId) {
    ...
    const idx = criterionTypes.value.findIndex(
...
const dialogCriteriaGroups = computed(() => {
    ...
    const list = allCriteria.value
        .filter((item) => {
            if (!q) return true;
            const hay =
                `${item.name ?? ""} ${item.criterion_type?.name ?? ""} ${item.criterion_type?.code ?? ""}`.toLowerCase();
...
const criterionEmptyMessage = computed(() =>
    allCriteria.value.length === 0
        ? "Phòng ban chưa có tiêu chí nào. Tạo tiêu chí mới ở tab Thêm."
        : "",
);

const criterionUnchanged = computed(() => {
    if (dialogTab.value !== "edit" || !selectedDialogCriterion.value)
        return false;
    const current = selectedDialogCriterion.value;
    const typeId = form.criterion_type_id
        ? Number(form.criterion_type_id)
        : null;
    const currentTypeId =
        current.criterion_type_id == null
            ? null
            : Number(current.criterion_type_id);
...
const hiddenActiveFilterLabels = computed(() =>
    FILTERS.filter(
        (item) => !visibleFilters[item.key] && filterHasValue(item.key),
    ).map((item) => item.label),
);
...
function computeDefaultWidths() {
    ...
    for (const col of shownColumns.value) {
        ...
        if (col.key === "status") {
            ...
            for (const row of allCriteria.value) {
                ...
                if (col.key === "level_count")
                    val = String(row.level_count ?? 0);
                if (col.key === "max_score") val = formatScore(row.max_score);
                if (col.key === "created_at")
                    val = formatDateTime(row.created_at) || "—";
    ...
    if (totalRaw < wrapWidth && shownColumns.value.length > 0) {
        ...
        for (const k in rawWidths)
            rawWidths[k] = Math.floor(rawWidths[k] * factor);
    }
...
async function downloadFile(
    ...
    try {
        ...
        const filename = decodeURIComponent(
            utfMatch?.[1] || plainMatch?.[1] || fallbackFilename,
        );
        ...
        if (err?.response?.data instanceof Blob) {
            try {
                ...
                message =
                    json.message || Object.values(json.errors || {})[0]?.[0];
            } catch {
                message = failMsg;
            }
        } else {
...
async function runPreview() {
    ...
    try {
        ...
        const { data } = await window.axios.post(
            "/api/evaluation/criteria/import/preview",
            formData,
            {
        ...
        if (err?.response?.status === 422) {
            showClientToast(
                "error",
                err.response.data?.message || "File không hợp lệ.",
            );
        } else {
            showClientToast(
                "error",
                err?.response?.data?.message || "Không đọc được file Excel.",
            );
        }
    } finally {
...
async function confirmImportRows() {
    const rows = (importPreview.value?.rows ?? [])
        .filter(
            (r) => r.status === "valid" && importSelectedRows.value.has(r.row),
        )
    ...
    try {
        const { data } = await window.axios.post(
            "/api/evaluation/criteria/import/confirm",
            { rows },
        );
        ...
        if (data.created?.length && !data.errors?.length) {
            showClientToast(
                "success",
                `Đã tạo thành công ${data.created.length} tiêu chí.`,
            );
        } else if (data.created?.length) {
            showClientToast(
                "warning",
                `Đã tạo ${data.created.length} tiêu chí, còn ${data.errors.length} dòng lỗi.`,
            );
        } else {
            showClientToast(
                "error",
                "Không tạo được tiêu chí nào, xem chi tiết lỗi bên dưới.",
            );
        }
    } catch (err) {
        showClientToast(
            "error",
            err?.response?.data?.message || "Không nhập được dữ liệu.",
        );
    } finally {
...
async function loadHistory() {
    ...
    try {
        const { data } = await window.axios.get(
            "/api/evaluation/criteria/history",
        );
        ...
        showClientToast(
            "error",
            err?.response?.data?.message || "Không tải được lịch sử thay đổi.",
        );
    } finally {
...
function openHistoryCriterion(log) {
    ...
    const criterion = allCriteria.value.find(
        (item) => String(item.id) === String(log.subject_id),
    );
    if (!criterion) {
...
async function saveCriterion() {
    ...
    try {
        if (dialogTab.value === "edit") {
            ...
            const idx = allCriteria.value.findIndex(
            ...
            showClientToast(
                "success",
                `Đã lưu tiêu chí "${data.criterion.name}".`,
            );
        } else {
            const { data } = await window.axios.post(
            ...
            showClientToast(
                "success",
                `Đã tạo tiêu chí "${data.criterion.name}".`,
            );
        }
        await nextTick();
        computeDefaultWidths();
        ...
        if (err?.response?.status === 422) {
            ...
            showClientToast(
                "error",
                err?.response?.data?.message || "Không lưu được tiêu chí.",
            );
        }
    } finally {
...
async function doDelete() {
    ...
    try {
        ...
        showClientToast(
            "error",
            err?.response?.data?.message || "Không xoá được tiêu chí.",
        );
    } finally {
...
async function toggleActive(criterion) {
    ...
    try {
        ...
        if (idx !== -1) allCriteria.value[idx] = data.criterion;
        if (selected.value?.id === criterion.id)
            selected.value = data.criterion;
    } catch (err) {
        showClientToast(
            "error",
    ...
    try {
        ...
        showClientToast(
            ...
            err?.response?.data?.message ||
                "Không cập nhật được dùng trong ĐGNL.",
        );
    } finally {
        togglingId.value = null;
    }
...
async function saveType() {
    ...
    try {
        const { data } = await window.axios.post(
            "/api/evaluation/criterion-types",
            {
        ...
        criterionTypes.value = [...criterionTypes.value, created].sort((a, b) =>
        ...
        if (err?.response?.status === 422) {
            ...
            showClientToast(
                "error",
                err?.response?.data?.message || "Không tạo được loại tiêu chí.",
            );
        }
    } finally {
...
function handleDocumentClick(event) {
    ...
    if (event.target?.closest?.(".eval-page__actions, .eval-page__action-menu"))
        return;
    closeActionMenu();
}
...
watch(
    () => form.id,
    (id) => {
        if (dialogKind.value !== "criterion" || dialogTab.value !== "edit")
            return;
        const criterion = allCriteria.value.find(
...
watch([query, kindFilter, typeFilter, statusFilter], () => {
    page.value = 1;
});

watch(visibleColumns, (value) => saveVisibility(COL_KEY, value), {
    deep: true,
});
watch(visibleFilters, (value) => saveVisibility(FILTER_KEY, value), {
    deep: true,
});
...
onMounted(async () => {
    ...
    if (tableWrap.value) {
        ...
        tableWrap.value.addEventListener("scroll", handleTableScroll, {
            passive: true,
        });
    }
});
    ...
    <div class="eval-page" :class="{ 'eval-page--with-panel': selected }">
        ...
        <div class="eval-page__main">
            <div v-if="hasVisibleFilterFields" class="eval-page__toolbar">
                <div class="eval-page__filters">
                    <div v-if="visibleFilters.q" class="eval-page__field">
                        <label class="eval-page__label" for="eval-q"
                            >Tìm kiếm</label
                        >
                        <input
                            id="eval-q"
                    ...
                    <div v-if="visibleFilters.kind" class="eval-page__field">
                        <label class="eval-page__label" for="eval-kind"
                            >Loại</label
                        >
                        <select
                            id="eval-kind"
                    ...
                    <div v-if="visibleFilters.type" class="eval-page__field">
                        <label class="eval-page__label" for="eval-type"
                            >Cách chấm</label
                        >
                        <select
                            id="eval-type"
                    ...
                    <div v-if="visibleFilters.status" class="eval-page__field">
                        <label class="eval-page__label" for="eval-status"
                            >Trạng thái</label
                        >
                        <select
                            id="eval-status"
            ...
            >
                <template #filters>
                    <label
                        ...
                        :key="item.key"
                        class="eval-page__check"
                    >
                        <input
                            type="checkbox"
                            :checked="visibleFilters[item.key]"
                            @change="
                                onFilterToggle(item.key, $event.target.checked)
                            "
                        />
                ...
                <template #settings>
                    ...
                    >
                        <input
                            ...
                            @change="
                                onColumnToggle(col.key, $event.target.checked)
                            "
                        />
                        <span>{{ col.label }}</span>
                    </label>
            ...
            <p v-if="hiddenActiveFilterLabels.length" class="eval-page__note">
                Đang lọc thêm theo:
                {{ hiddenActiveFilterLabels.join(", ") }} (bộ lọc đang ẩn).
            </p>
                ...
                <table v-else class="eval-page__table">
                    <colgroup>
                        <col
                            ...
                            :style="{
                                width:
                                    (colWidths[col.key] ?? MIN_COL_PX) + 'px',
                            }"
                        />
                        ...
                        <tr>
                            ...
                            >
                                ...
                                <button
                                    ...
                                    @mousedown.stop="
                                        startResize($event, col.key)
                                    "
                                />
                            </th>
                            <th
                                v-if="canManage"
                                class="eval-page__th eval-page__th--center"
                            >
                                Thao tác
                            </th>
                        </tr>
                        ...
                        >
                            ...
                            >
                                <td
                                    ...
                                    class="eval-page__td eval-page__td--group"
                                >
                                    <div
                                        class="eval-page__group-inner"
                                        :class="
                                            entry.scoringAccent
                                                ? `eval-page__group-inner--${entry.scoringAccent}`
                                                : ''
                                        "
                                    >
                                        <span
                                            class="eval-page__group-toggle"
                                            aria-hidden="true"
                                        >
                                            <AppIcon
                                                :name="
                                        ...
                                        <span class="eval-page__group-copy">
                                            <span
                                                class="eval-page__group-title"
                                                >{{ entry.title }}</span
                                            >
                                            <span
                                                v-if="entry.code"
                                                class="eval-page__group-code"
                                                >{{ entry.code }}</span
                                            >
                                        </span>
                                        <span
                                            v-if="entry.scoringKinds.length"
                                            class="eval-page__group-badges"
                                        >
                                            <span
                                                ...
                                                :class="
                                                    'eval-page__badge--' + kind
                                                "
                                            >
                                                {{ TYPE_LABELS[kind] ?? kind }}
                                            </span>
                                        </span>
                                        <span class="eval-page__group-count"
                                            >{{ entry.count }} tiêu chí</span
                                        >
                                    </div>
                                </td>
                            </tr>
                            <tr
                                ...
                                class="eval-page__tr"
                                :class="{
                                    'eval-page__tr--active':
                                        selected?.id === entry.criterion.id,
                                }"
                            ...
                            >
                                ...
                                >
                                    <span
                                        ...
                                        :class="
                                            entry.criterion.is_active
                                                ? 'eval-page__badge--active'
                                                : 'eval-page__badge--inactive'
                                        "
                                    >
                                        {{
                                            entry.criterion.is_active
                                ...
                                >
                                    <button
                                        ...
                                        role="switch"
                                        :aria-checked="
                                            entry.criterion.use_in_evaluation
                                                ? 'true'
                                                : 'false'
                                        "
                                ...
                                >
                                    <UserAvatarTip
                                        :user="entry.criterion.creator"
                                        label="Người tạo"
                                    />
                                </td>
                                <td
                                    v-if="visibleColumns.updater"
                                    class="eval-page__td eval-page__td--center"
                                >
                                    <UserAvatarTip
                                        :user="entry.criterion.updater"
                                        label="Người cập nhật"
                                    />
                                </td>
                                <td
                                    v-if="visibleColumns.description"
                                    class="eval-page__td eval-page__td--desc"
                                >
                                    {{ entry.criterion.description || "—" }}
                                </td>
                                <td
                                    v-if="canManage"
                                    class="eval-page__td eval-page__td--center eval-page__td--action"
                                    @click.stop
                                >
        ...
        >
            ...
            <div class="eval-page__panel-body hide-scrollbar">
                <dl class="eval-page__dl">
                    ...
                    <div
                        v-if="selected.criterion_type?.code"
                        class="eval-page__dl-row"
                    >
                        <dt>Mã loại</dt>
                        <dd>{{ selected.criterion_type.code }}</dd>
                    </div>
                    <div
                        v-if="selected.criterion_type?.description"
                        class="eval-page__dl-row"
                    >
                        <dt>Mô tả loại</dt>
                    ...
                    <div class="eval-page__dl-row">
                        ...
                        <dd>
                            <span
                                ...
                                :class="'eval-page__badge--' + selected.type"
                            >
                                {{
                ...
                <ul class="eval-page__levels-list">
                    ...
                    >
                        <span class="eval-page__level-copy">
                            ...
                            <span
                                v-if="lv.description"
                                class="eval-page__level-note"
                                >{{ lv.description }}</span
                            >
                        </span>
                        <span
                            class="eval-page__level-score"
                            :class="
                                lv.score < 0
                                    ? 'eval-page__level-score--neg'
                                    : ''
                            "
                        >
                            {{ lv.score > 0 ? "+" : ""
                    ...
                    >
                        <div class="eval-page__dialog-head">
                            ...
                            <div class="eval-page__dialog-head-copy">
                                <h2
                                    id="eval-criterion-form-title"
                                    class="eval-page__dialog-title"
                                >
                                    {{ criterionDialogTitle }}
                                </h2>
                            </div>
                            <button
                                type="button"
                        ...
                        <div
                            class="eval-page__dialog-tabs"
                            role="tablist"
                            aria-label="Thêm, sửa hoặc lịch sử tiêu chí"
                        >
                            <button
                                type="button"
                        ...
                        <div
                            ...
                            :class="{
                                'eval-page__dialog-body--edit':
                                    dialogTab === 'edit',
                                'eval-page__dialog-body--history':
                                    dialogTab === 'history',
                            }"
                        >
                            <ol
                            ...
                            >
                                <li
                                    v-if="historyLoading"
                                    class="eval-page__history-empty"
                                >
                                    Đang tải…
                                </li>
                                <li
                                    ...
                                    >
                                        <span
                                            ...
                                            :class="
                                                ...
                                                historyVerbTone(log.action)
                                            "
                                            aria-hidden="true"
                                        >
                                            <AppIcon
                                                :name="
                                                    historyVerbIcon(log.action)
                                                "
                                                :size="13"
                                                :stroke-width="1.75"
                                            />
                                        ...
                                        <div class="eval-page__history-card">
                                            <div
                                                class="eval-page__history-head"
                                            >
                                                <div
                                                    class="eval-page__history-who"
                                                >
                                                    <span
                                                        class="eval-page__history-name"
                                                        >{{
                                                            log.actor_name
                                                        }}</span
                                                    >
                                                    <span
                                                        class="eval-page__history-verb"
                                                        >{{ log.verb }}</span
                                                    >
                                                    <button
                                                        v-if="log.can_open"
                                                ...
                                                <span
                                                    class="eval-page__history-avatar"
                                                    aria-hidden="true"
                                                >
                                                    <img
                                                        v-if="
                                            ...
                                            <p
                                                class="eval-page__history-detail"
                                            >
                                                {{ log.detail }}
                                            </p>
                                        </div>
                            ...
                            <template v-else>
                                <div
                                    v-if="dialogTab === 'edit'"
                                    class="eval-page__dialog-list-panel"
                                >
                                    <label
                                        class="eval-page__dialog-label"
                                        for="eval-criterion-list-q"
                                        >Tiêu chí</label
                                    >
                                    <input
                                    ...
                                    />
                                    <ul
                                        class="eval-page__dialog-list hide-scrollbar"
                                        role="listbox"
                                        aria-label="Danh sách tiêu chí"
                                    >
                                        <li
                                            v-if="allCriteria.length === 0"
                                            class="eval-page__dialog-list-empty"
                                        >
                                            {{ criterionEmptyMessage }}
                                        </li>
                                        <li
                                            v-else-if="!dialogListHasItems"
                                            class="eval-page__dialog-list-empty"
                                        >
                                        ...
                                        <template v-else>
                                            <template
                                                v-for="group in dialogCriteriaGroups"
                                                :key="group.key"
                                            >
                                                <li
                                                    class="eval-page__dialog-list-group"
                                                >
                                                    <button
                                                        type="button"
                                                        ...
                                                        :class="
                                                            groupScoringAccent(
                                                                ...
                                                                ? `eval-page__dialog-list-group-head--${groupScoringAccent(group.criteria)}`
                                                                : ''
                                                        "
                                                        :aria-expanded="
                                                            !isTypeGroupCollapsed(
                                                                dialogTypeGroupCollapsed,
                                                                group.key,
                                                            )
                                                                ? 'true'
                                                                : 'false'
                                                        "
                                                        @click="
                                                            toggleDialogTypeGroup(
                                                                group.key,
                                                            )
                                                        "
                                                    >
                                                        <span
                                                            class="eval-page__dialog-list-group-toggle"
                                                            aria-hidden="true"
                                                        >
                                                            <AppIcon
                                                                :name="
                                                        ...
                                                        <span
                                                            class="eval-page__dialog-list-group-copy"
                                                        >
                                                            <span
                                                                class="eval-page__dialog-list-group-title"
                                                                >{{
                                                                    groupTitle(
                                                                        group.type,
                                                                    )
                                                            ...
                                                            <span
                                                                v-if="
                                                                    group.type
                                                                        ?.code
                                                                "
                                                                class="eval-page__dialog-list-group-code"
                                                                >{{
                                                                    group.type
                                                                        .code
                                                                }}</span
                                                        ...
                                                        <span
                                                            class="eval-page__dialog-list-group-count"
                                                            >{{
                                                                group.criteria
                                                                    .length
                                                            }}</span
                                                ...
                                                >
                                                    <span
                                                        class="eval-page__dialog-list-copy"
                                                    >
                                                        <span
                                                            class="eval-page__dialog-list-name"
                                                            >{{
                                                                criterion.name
                                                            }}</span
                                                        >
                                ...
                                <div
                                    ...
                                    :class="{
                                        'eval-page__form--disabled':
                                            dialogTab === 'edit' && !form.id,
                                    }"
                                >
                                    <div class="eval-page__form-info">
                                        <div
                                            class="eval-page__form-field eval-page__form-field--name"
                                        >
                                            <label
                                                class="eval-page__form-label"
                                                for="eval-criterion-name"
                                            >
                                                Tên tiêu chí
                                                <span
                                                    class="eval-page__required"
                                                    >*</span
                                                >
                                            </label>
                                            <input
                                        ...
                                        <div
                                            class="eval-page__form-field eval-page__form-field--kind"
                                        >
                                            <label
                                                class="eval-page__form-label"
                                                for="eval-criterion-type"
                                                >Loại tiêu chí</label
                                            >
                                        ...
                                        <div
                                            class="eval-page__form-field eval-page__form-field--scoring"
                                        >
                                            <span
                                                class="eval-page__form-label"
                                                id="eval-criterion-scoring-label"
                                            >
                                                Cách chấm
                                                <span
                                                    class="eval-page__required"
                                        ...
                                        <div
                                            class="eval-page__form-field eval-page__form-field--desc"
                                        >
                                            <label
                                                class="eval-page__form-label"
                                                for="eval-criterion-desc"
                                                >Mô tả</label
                                            >
                                            <textarea
                                        ...
                                        <div
                                            class="eval-page__form-field eval-page__form-field--status"
                                        >
                                            <span
                                                class="eval-page__form-label"
                                                id="eval-criterion-status-label"
                                                >Trạng thái</span
                                            >
                                            <div
                                                class="eval-page__seg"
                                            ...
                                            <div class="eval-page__half-toggle">
                                                <span
                                                    id="eval-use-dgnl-label"
                                                    class="eval-page__half-toggle-label"
                                                    >Dùng trong ĐGNL</span
                                                >
                                                <button
                                                    type="button"
                                                    class="eval-page__switch"
                                    ...
                                    >
                                        <div class="eval-page__scale-head">
                                            ...
                                            <div
                                                class="eval-page__scale-actions"
                                            >
                                                <div
                                                    class="eval-page__half-toggle"
                                                >
                                                    <span
                                                        id="eval-allow-half-label"
                                                        class="eval-page__half-toggle-label"
                                                        >Chấm 0.5</span
                                                    >
                                                    <button
                                        ...
                                        >
                                            {{
                                                Array.isArray(formErrors.levels)
                                                    ? formErrors.levels[0]
                                                    : formErrors.levels
                                            }}
                                        </div>
                                        ...
                                        <div
                                            class="eval-page__levels-form hide-scrollbar"
                                        >
                                            <div
                                                class="eval-page__levels-head"
                                                aria-hidden="true"
                                            >
                                                <span>Mã mức</span>
                                                <span>Nhãn mức</span>
                        ...
                        <div class="eval-page__dialog-actions">
                            <button
                                type="button"
                                class="eval-page__dialog-btn eval-page__dialog-btn--ghost"
                                :disabled="formSaving"
                                @click="closeDialog"
                            >
                ...
                >
                    ...
                    >
                        <div class="eval-page__dialog-head">
                            ...
                            <div class="eval-page__dialog-head-copy">
                                <h2
                                    id="eval-type-form-title"
                                    class="eval-page__dialog-title"
                                >
                                    Thêm loại tiêu chí mới
                                </h2>
                            </div>
                            <button
                                type="button"
                        ...
                        <div class="eval-page__dialog-actions">
                            ...
                            <button
                                ...
                                class="eval-page__dialog-btn eval-page__dialog-btn--primary"
                                :disabled="
                                    typeFormSaving || !typeForm.name.trim()
                                "
                                @click="saveType"
                            >
                                {{ typeSubmitLabel }}
                ...
                >
                    ...
                    >
                        <div class="eval-page__dialog-head">
                            ...
                            <div class="eval-page__dialog-head-copy">
                                <h2
                                    id="eval-import-title"
                                    class="eval-page__dialog-title"
                                >
                                    Nhập tiêu chí từ Excel
                                </h2>
                            </div>
                            <button
                                type="button"
                        ...
                        <template v-if="importStep === 'select'">
                            <div class="eval-page__dialog-body">
                                <p class="eval-page__import-hint">
                                    Chọn file Excel (.xlsx) đúng theo cấu trúc
                                    cột đã xuất. Loại tiêu chí trong file phải
                                    được tạo sẵn trong phòng ban trước khi nhập.
                                    Sau khi tải lên, hệ thống sẽ hiện bảng xem
                                    trước để bạn kiểm tra trước khi ghi dữ liệu
                                    thật.
                                    <button
                                        type="button"
                                        class="eval-page__import-template"
                                        :disabled="exporting"
                                        @click="exportExcel"
                                    >
                        ...
                        <template v-else-if="importStep === 'preview'">
                            <div
                                class="eval-page__dialog-body eval-page__dialog-body--preview"
                            >
                                <div class="eval-page__import-summary">
                                    <span
                                        class="eval-page__import-dot eval-page__import-dot--ok"
                                    />
                                    {{ importSelectedRows.size }}/{{
                                        importPreview?.rows?.length ?? 0
                                    }}
                                    dòng hợp lệ được chọn để nhập.
                                </div>

                                <div
                                    class="eval-page__preview-table-wrap hide-scrollbar"
                                >
                                    <table class="eval-page__preview-table">
                                        <thead>
                                            <tr>
                                                <th
                                                    class="eval-page__preview-th eval-page__preview-th--check"
                                                ></th>
                                                <th
                                                    class="eval-page__preview-th"
                                                >
                                                    Dòng
                                                </th>
                                                <th
                                        ...
                                        <tbody>
                                            <tr
                                                ...
                                                :class="{
                                                    'eval-page__preview-row--invalid':
                                                        row.status !== 'valid',
                                                }"
                                            >
                                                <td
                                                    class="eval-page__preview-td eval-page__preview-td--check"
                                                >
                                                    <input
                                                        v-if="
                                                            row.status ===
                                                ...
                                                >
                                                    <span
                                                        class="eval-page__import-summary"
                                                    >
                                                        <span
                                                            ...
                                                            :class="
                                                                row.status ===
                                                                'valid'
                                                                    ? 'eval-page__import-dot--ok'
                                                                    : 'eval-page__import-dot--error'
                                                            "
                                                        />
                                                        {{
                                                            row.status ===
                                                ...
                                                <td
                                                    class="eval-page__preview-td eval-page__preview-td--issues"
                                                >
                                                    <p
                                                        v-for="(
                                                            issue, idx
                                                        ...
                                                        :key="idx"
                                                        class="eval-page__preview-issue"
                                                    >
                                                        {{ issue.message }}
                                                    </p>
                                                </td>
                            ...
                            <div class="eval-page__dialog-actions">
                                ...
                                >
                                    {{
                                        confirming
                                            ? "Đang nhập…"
                                            : `Xác nhận nhập (${importSelectedRows.size})`
                                    }}
                                </button>
                            </div>
                        </template>
                        ...
                        <template v-else>
                            <div class="eval-page__dialog-body">
                                <div
                                    v-if="importResult"
                                    class="eval-page__import-result"
                                >
                                    <div class="eval-page__import-summary">
                                        <span
                                            class="eval-page__import-dot eval-page__import-dot--ok"
                                        />
                                        Đã tạo thành công
                                        {{ importResult.created.length }} tiêu
                                        chí
                                        <template
                                    ...
                                    <ul
                                        v-if="importResult.errors.length"
                                        class="eval-page__import-errors"
                                    >
                                        <li
                                            v-for="err in importResult.errors"
                                            :key="err.row"
                                            class="eval-page__import-error"
                                        >
                                            <span
                                                class="eval-page__import-dot eval-page__import-dot--error"
                                            />
                                            Dòng {{ err.row }}:
                                            {{ err.message }}
                                        </li>
        ...
        <div
            v-if="confirmDelete"
            class="eval-page__confirm-overlay"
            @click.self="confirmDelete = null"
        >
            <div
                class="eval-page__confirm"
                role="alertdialog"
                aria-modal="true"
            >
                ...
                <p class="eval-page__confirm-msg">
                    ...
                    <strong>«{{ confirmDelete.name }}»</strong>? Hành động này
                    không thể hoàn tác.
                </p>
...
.eval-page__toolbar-spin {
    animation: eval-page-toolbar-spin 0.8s linear infinite;
}
...
.eval-page__th:hover .eval-page__resize::after {
    opacity: 1;
}
...
.eval-page__badge--scale {
    background: color-mix(in srgb, var(--color-primary) 12%, transparent);
    color: var(--color-primary);
}
.eval-page__badge--behavior {
    background: color-mix(
        in srgb,
...
.eval-page__icon-btn--ghost {
    color: var(--color-text-muted);
}
.eval-page__icon-btn--ghost:hover:not(:disabled) {
    background: var(--color-surface-muted);
    color: var(--color-text);
}
.eval-page__icon-btn--danger {
    color: var(--color-danger, #dc2626);
}
.eval-page__icon-btn--danger:hover:not(:disabled) {
    background: color-mix(
        in srgb,
        var(--color-danger, #dc2626) 10%,
        transparent
...
.eval-page__panel-btn:hover:not(:disabled) {
    background: var(--color-surface);
}
...
.eval-page__level-item:last-child {
    box-shadow: none;
}
...
.eval-page__form-field--name {
    grid-area: name;
}
.eval-page__form-field--kind {
    grid-area: type;
}
.eval-page__form-field--scoring {
    grid-area: scoring;
}
.eval-page__form-field--desc {
    grid-area: desc;
}
.eval-page__form-field--status {
    grid-area: status;
}
...
.eval-page__level-row {
    display: grid;
    grid-template-columns: 5.25rem minmax(8rem, 1fr) minmax(
            10rem,
            1.5fr
        ) 5.25rem 2rem;
    gap: var(--space-2);
...
.eval-page__add-level-btn:hover {
    background: var(--color-primary-surface-strong);
}
.eval-page__add-level-btn:disabled {
    opacity: 0.5;
    cursor: not-allowed;
}
...
.eval-page__btn:hover:not(:disabled) {
    filter: brightness(0.95);
}
.eval-page__btn:disabled {
    opacity: 0.6;
    cursor: not-allowed;
}
...
.eval-page__preview-td--check {
    text-align: center;
}

.eval-page__preview-td--issues {
    min-width: 16rem;
}
...
.eval-page__preview-issue + .eval-page__preview-issue {
    margin-top: 0.125rem;
}
...
@media (max-width: 640px) {
    ...
    .eval-page__levels-head,
    .eval-page__level-row {
        grid-template-columns: 4.5rem minmax(7rem, 1fr) minmax(
                8rem,
                1.2fr
            ) 4.5rem 2rem;
    }
...
@media (max-width: 480px) {
    ...
    .eval-page__preview-table td:nth-child(4) {
        display: none;
    }
}
