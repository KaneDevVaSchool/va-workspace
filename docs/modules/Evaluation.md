...
| `evaluation_template_criteria` | `EvaluationTemplateCriterion` | Pivot N-N `evaluation_templates` ↔ `evaluation_criteria`, mang `weight_label`/`weight_value` (trọng số riêng theo từng mẫu, hiển thị chữ tiếng Việt — không lộ số ra UI), `required_score`, `count_in_total`. |
